package seedu.canvas.storage;

public class FilePath {

    /* Path to Resource Directory */
    public static final String RESOURCE_PATH = "src\\main\\resources";


    /* Path to Scenes */
    public static final String MENU_SCENE_PATH = "/scenes/menu.fxml";
    public static final String CANVAS_SCENE_PATH = "/scenes/canvas.fxml";


    /* Path to Styles */
    public static final String MENU_STYLE_PATH = "/styles/menu.css";
    public static final String CANVAS_STYLE_PATH = "/styles/canvas.css";
    public static final String TEXT_STYLE_PATH = "/styles/text.css";


    /* Path to Button Images */
    public static final String MENU_NEW_BUTTON_IMAGE_PATH = "/images/icons/new_button.png";
    public static final String MENU_OPEN_BUTTON_IMAGE_PATH = "/images/icons/open_button.png";
    public static final String MENU_HELP_BUTTON_IMAGE_PATH = "/images/icons/help_button.png";
    public static final String CANVAS_POINT_BUTTON_IMAGE_PATH = "/images/icons/point_button.png";

    public static final String CANVAS_MODEL_BUTTON_IMAGE_PATH = "/images/icons/model_button.png";
    public static final String CANVAS_MODEL_OPTION_BUTTON_IMAGE_PATH = "/images/icons/model_button_2.png";
    public static final String CANVAS_LINE_BUTTON_IMAGE_PATH = "/images/icons/line_button.png";
    public static final String CANVAS_LINE_OPTION_BUTTON_IMAGE_PATH = "/images/icons/line_button_2.png";
    public static final String CANVAS_ANCHOR_LINE_BUTTON_IMAGE_PATH = "/images/icons/anchor_line_button.png";
    public static final String CANVAS_ANCHOR_LINE_OPTION_BUTTON_IMAGE_PATH = "/images/icons/anchor_line_button_2.png";
    public static final String CANVAS_GROUP_LINE_BUTTON_IMAGE_PATH = "/images/icons/group_line_button.png";
    public static final String CANVAS_GROUP_LINE_OPTION_BUTTON_IMAGE_PATH = "/images/icons/group_line_button_2.png";

    public static final String CANVAS_TEXT_BUTTON_IMAGE_PATH = "/images/icons/text_button.png";
    public static final String CANVAS_BOLD_BUTTON_IMAGE_PATH = "/images/icons/bold_button.png";
    public static final String CANVAS_ITALIC_BUTTON_IMAGE_PATH = "/images/icons/italic_button.png";
    public static final String CANVAS_UNDERLINE_BUTTON_IMAGE_PATH = "/images/icons/underline_button.png";
    public static final String CANVAS_STRIKETHROUGH_BUTTON_IMAGE_PATH = "/images/icons/strikethrough_button.png";


    public static final String CANVAS_DRAW_BUTTON_IMAGE_PATH = "/images/icons/draw_button.png";

    public static final String CANVAS_COLOUR_BUTTON_IMAGE_PATH = "/images/icons/colour_button.png";
    public static final String CANVAS_COLOUR_LINE_BUTTON_IMAGE_PATH = "/images/icons/colour_line_button.png";
    public static final String CANVAS_COLOUR_FILL_BUTTON_IMAGE_PATH = "/images/icons/colour_fill_button.png";


    /* Path to Miscellaneous Images */
    public static final String CANVAS_ICON_IMAGE_PATH = "/images/misc/canvas_icon.png";
    public static final String CANVAS_LOGO_IMAGE_PATH = "/images/misc/canvas_logo.png";

}
